...
class FileProcessor {
  ...
  updateDependecies() {
    let { publicURL, outDir } = this.config;
    const { oldFilePath, newFilePath, folderPath } = this;

    // manifest file should contain relative links to other files
    // not absolute (hence publicURL)
    if (isManifestFile(oldFilePath)) publicURL = '';
    // If the file that is not in the graph was provided (like .DS_Store)
    if (!this.depGraph.hasNode(oldFilePath)) return;
  ...
  updateDependants() {
    ...
    for (const depPath of fileDependantsPaths) {
      ...
      if (isManifestFile(depPath)) publicURL = '';
